...
class VanillaRNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.hidden_linear = nn.Linear(self.input_size + self.hidden_size, self.hidden_size)
        self.tanh = nn.Tanh()

        # initialize block that creates output
        self.output_linear = nn.Linear(self.input_size + self.hidden_size, self.output_size)
    ...
    def forward(self, input, hidden):
        ...
        input_concat_hidden = torch.cat((input,hidden), 1)

        output = self.logSoftmax(self.output_linear(input_concat_hidden))

        hidden = self.tanh(self.hidden_linear(input_concat_hidden))
