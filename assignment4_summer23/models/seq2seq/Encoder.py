...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size, emb_size, encoder_hidden_size, decoder_hidden_size, dropout=0.2, model_type="RNN"):
        ...
        if model_type == "RNN":
            ...
            self.lstm = nn.LSTM(emb_size, encoder_hidden_size, batch_first = True)
        else:
    ...
    def forward(self, input):
        ...
        hinit = torch.zeros(1, N, self.encoder_hidden_size).to(input.device)

        print("model type is ", self.model_type)

        if self.model_type == "RNN":
            output, hidden = self.rnn(drop)


        elif self.model_type == "LSTM":
            output, (hidden, cell) = self.lstm(drop)


        hidden = self.tanh(self.Linear2(self.relu(self.Linear1(hidden))))
